...
id_map_active=$(mysql -D $dbname -u$username --password="$password" -N -e "SELECT id_beamng FROM $tablename WHERE map_active = 1;")

# Variable map actuelle officielle ou pas

official_map_active=$(mysql -D $dbname -u$username --password="$password" -N -e "SELECT map_officielle FROM $tablename WHERE map_active = 1;")

# Variable nouvelle map officielle ou pas

official_new_map=$(mysql -D $dbname -u$username --password="$password" -N -e "SELECT map_officielle FROM $tablename WHERE id_beamng = '$1';")

# Variable chemin du zip de la map active

chemin_zip_map_active=$(mysql -D $dbname -u$username --password="$password" -N -e "SELECT chemin FROM $tablename WHERE map_active = 1;")

# Variable chemin du zip de la nouvelle map

chemin_zip_new_map=$(mysql -D $dbname -u$username --password="$password" -N -e "SELECT chemin FROM $tablename WHERE id_beamng = '$1';")

# Variable nom du zip de la map active

nom_zip_map_active=$(mysql -D $dbname -u$username --password="$password" -N -e "SELECT chemin FROM $tablename WHERE map_active = 1;" | cut -d "/" -f8)

# Variable nom du zip de la nouvelle map

nom_zip_new_map=$(mysql -D $dbname -u$username --password="$password" -N -e "SELECT chemin FROM $tablename WHERE id_beamng = '$1';" | cut -d "/" -f8)
...
if [ "$official_map_active" -eq 0 ]; then
    mysql -D $dbname -u$username --password="$password" -e "UPDATE ${dbname}.${tablename} SET chemin = REPLACE(chemin, 'Client', 'inactive_map') WHERE id_beamng = '$id_map_active';"
fi

# On met à jour dans la base de données le chemin de la nouvelle map pour que ça corresponde au chemin de la map active

if [ "$official_new_map" -eq 0 ]; then
    mysql -D $dbname -u$username --password="$password" -e "UPDATE ${dbname}.${tablename} SET chemin = REPLACE(chemin, 'inactive_map', 'Client') WHERE id_beamng = '$1';"
fi

# Mise à 0 de la valeur map_active pour la map active

mysql -D $dbname -u$username --password="$password" -e "UPDATE $tablename SET map_active = 0 WHERE id_beamng = '$id_map_active';"

# Valeur map_active à 1 pour la map choisi

mysql -D $dbname -u$username --password="$password" -e "UPDATE $tablename SET map_active = 1 WHERE id_beamng = '$1';"
